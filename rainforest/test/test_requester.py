...
class TestRequesterGetHeadDelete(TestRequester):
	def test_convert_get_params(self):
		url, params = rainforest.apibits.Requester.prepare_params(self.url, {}, self.params, "get")
		self.assertNotEqual(url, self.url)
		self.assertIsNone(params)

	def test_convert_delete_params(self):
		url, params = rainforest.apibits.Requester.prepare_params(self.url, {}, self.params, "delete")
		self.assertNotEqual(url, self.url)
		self.assertIsNone(params)

	def test_convert_head_params(self):
		url, params = rainforest.apibits.Requester.prepare_params(self.url, {}, self.params, "head")
		self.assertNotEqual(url, self.url)
		self.assertIsNone(params)
...
class TestRequesterPost(TestRequester):
	...
	def test_convert_post_params(self):
		url, params = rainforest.apibits.Requester.prepare_params(self.url, {}, self.params, self.method)
...
class TestRequesterQuery(TestRequester):
	def test_query_array_join(self):
		...
		actual = rainforest.apibits.Requester.query_string(data).split('&')
		for param in actual:
	...
	def test_query_array_flat(self):
		data = {"a" : "value"}
		expected = ["a=value"]
		self.assertEqual(expected, rainforest.apibits.Requester.query_array(data))
